...
> 🔴 **위 코드는 결함이 있다. 수정 라운드 1 에서 교체됐다 (2026-08-27).**
>
> `except urllib.error.HTTPError` 가 `sys.exit()` 없이 끝나 **암묵적 exit 0** 이 된다.
> 401(키 오류)·403·404(URL·테이블 오탈자)·500(서버 장애)이 전부 「차단됨(성공)」으로
> 보고된다. **점검 도구가 고장났을 때 「안전합니다」라고 말한다.**
> `total == "*"`(카운트 미확정)을 0행과 같이 취급하는 것도 같은 종류의 오판이다.
>
> **확정 설계 — 종료코드 3분기.** 「모르겠다」를 「안전하다」와 구분하는 것이 핵심이다:
> `0 = 차단됨` / `1 = 노출됨` / `2 = 판정 불가`.
> 판정 로직을 `classify(rows_returned, content_range)` 순수 함수로 분리하고
> `scripts/test_verify_app_jobs_rls.py` 가 그것만 테스트한다(네트워크 모킹 없음).
>
> 실제 구현은 커밋된 `scripts/verify_app_jobs_rls.py` 를 보라. 이 계획서를 다시 실행하는
> 사람은 위 코드 블록이 아니라 **저장소의 현재 파일**을 기준으로 삼아야 한다.
>
> **Task 2 가 이 스크립트를 CI 에 넣을 때 주의:** 종료코드 2 도 0 이 아니므로 CI 는
> 실패로 잡는다. 그게 의도된 동작이다 — 판정 불가는 통과가 아니다.
