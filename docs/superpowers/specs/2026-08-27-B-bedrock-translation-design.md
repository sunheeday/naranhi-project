...
- **문서판독(멀티모달) 이전** — `GeminiDocumentExtractor`
  ...
  다만 **«기술적으로 불가능해서 안 하는 것»이 아니다.** 2026-08-27 실측으로
  Bedrock `Converse`가 PDF·이미지를 직접 받아 정확히 읽는다는 것이 확인됐다(§5.9.2).
  이전 가능함은 **확정된 사실**이고, 실제 이전은 **사업 C(판독 영역)** 로 넘긴다. §10.4 참조.
...
**모델 선택 정책(§5.10)이 arm 순서를 결정한다 — Haiku 우선, Sonnet은 승급용이다.**

| arm | 백엔드 | 목적 |
|---|---|---|
| 기준선 | Gemini 2.5 Flash, 현행 그대로 | 비교 기준 (교체하지 않는다) |
| T | Gemini 2.5 Flash, **thinking 전면 0** | B4의 품질 영향 단독 측정 |
| **H** | Bedrock `global.anthropic.claude-haiku-4-5-20251001-v1:0` | **1순위 — 기본 후보** |
| S | Bedrock `global.anthropic.claude-sonnet-4-6` | **승급 후보.** H가 게이트를 못 넘을 때만 채택 |
| M | Bedrock 혼합 (기계 단계 Haiku + 번역 단계 Sonnet) | `source_hard_fact_model` 분리 활용 |
...
> **Nova arm은 이 축에 없다.** 2026-08-27 실측에서 Nova Lite·Nova Pro가
> **읽지 못한 문서의 본문을 지어냈다**(§5.9.2). 사용자에게 보이는 한국어 텍스트를
> 생성하는 경로에서는 후보가 아니다. Nova의 남은 쓸모는 §5.10.3에 용도 제한으로 적는다.
>
> **Opus 계열도 이 축에 없다.** 사용자 지시로 **금지**다(§5.10.2).
...
| **G6** | **환각 negative control 통과** — 판독 불가능한 입력에 대해 «못 읽겠다»고 답한다. 지어낸 본문 **0건** |

G1이 «하락 0건»인 이유: 36건 표본에서 통과율 비율 차이는 의미가 없지만
**"전에 통과하던 건이 지금 실패한다"는 개별 사건**은 표본 크기와 무관하게 실재하는 회귀다.

### 4.7 G6 — 환각은 «번역 품질»과 다른 축이다

G1~G5는 전부 **모델이 입력을 읽었다는 전제** 위에서 잰다. 2026-08-27 실측(§5.9.2)이
보여준 실패 모드는 그 전제 자체가 깨지는 경우다 — 모델이 읽지 못했는데 **«못 읽겠다»고
말하는 대신 그럴듯한 가정통신문을 지어냈다.** 이 산출물은 8축 평가에서 오히려
높은 점수를 받을 수 있다. 문장이 매끄럽고 형식이 맞기 때문이다.

> **따라서 «지어내는가»는 «잘 번역하는가»와 별도로 재야 한다.**
> 방법은 negative control이다 — 정상 코퍼스에 **판독 불가능한 입력을 의도적으로 섞고**,
> 모델이 거부하는지 지어내는지를 센다. 채점은 이분법이다: 거부 = 통과, 본문 생성 = 실패.
> 한 건이라도 지어내면 그 arm은 사용자에게 보이는 한국어 생성 경로에서 탈락한다.
...
| Nova | — | 무관 — Nova는 이 파이프라인의 후보가 아니다(§5.10.3) |
...
- 여기에 assistant 프리필 `"{"`를 얹는다. 그러면 응답 텍스트가 `{` **다음부터** 오므로
  ...
  (Claude 계열이 프리필을 어떻게 다루는지는 **미확인** — 프리필을 무시하고 완전한 JSON을
  돌려줄 수도 있다. B1 평가에서 파싱 실패 건수를 세어 판단한다.)
...
# 기본은 Haiku 다 (§5.10). Sonnet 승급은 env 한 줄로 한다.
...
계정 `554608989606`, 자격 `naranhi-bedrock` IAM. 액세스 신청 없이 즉시 호출 가능한 것이
**12개**임을 확인했다. 아래 표에서 **Opus 계열 2종은 뺐다** — 호출은 되지만
**후보가 아니다**(§5.10.2, 사용자 지시).

#### 5.9.1 텍스트 전용 지연 (초경량 프롬프트)
...
같은 날 재측정(초경량 프롬프트): **Haiku 4.5 920ms / Sonnet 4.6 1,303ms / Nova Lite 341ms.**
회차 간 편차 범위 안이고 **순서는 뒤집히지 않는다** — Nova < Haiku < Sonnet.

막힌 것(계정 미승인): `claude-sonnet-5`, `claude-fable-5`, `openai.gpt-5.6-*`, `xai.grok-4.6`.

`global.` 접두는 요청이 APAC 밖으로 나갈 수 있다는 뜻이다. **이 항목은 사용자가 허용했다**
(§15 결정 5, §16 해결됨 Q1). 사실만 기록으로 남긴다 — 학교 공지에는 학생 이름·학년반·
보호자 연락처가 섞이고, `global.` arm을 돌리면 그 데이터가 APAC 밖으로 나간다.

#### 5.9.2 운영 첨부 2건을 직접 던진 판독 시험 — **Nova가 지어냈다**

운영 DB의 **실제 학교 공지 첨부 2건**을 Bedrock `Converse`에 그대로 넣었다.
조건: 리전 `ap-northeast-2`, `maxTokens=700`, `temperature=0`.

**시험 1 — 이미지 (`본문 사진.png`, 9KB, 실제 내용은 QR 코드 하나)**

| 모델 | 응답 | 판정 |
|---|---|---|
| Claude Sonnet 4.6 | "이 이미지는 QR 코드입니다. 가정통신문 텍스트가 아니므로 읽어드리기 어렵습니다" | ✅ **정직** |
| Claude Haiku 4.5 | "QR 코드 이미지만 제공되어 직접 읽을 수 없습니다" | ✅ **정직** |
| **Nova Lite** | "안녕하세요. **교장 김 교장입니다.** 새 학기를 맞이하여…" — **표까지 생성** | 🔴 **환각** |
| **Nova Pro** | "안녕하세요. 이번 학기도 잘 시작되었습니다…" | 🔴 **환각** |

**시험 2 — PDF (`안내문.pdf`, 1,219KB, 「2026 인천농어촌유학 말랑갯티학교 2학기 장기체류형」)**

| 모델 | 지연 | 한글 | 표 | 판정 |
|---|---|---|---|---|
| Claude Sonnet 4.6 | 15,929ms | 371자 | 인식 | ✅ **정확** — "인천광역시교육청 2026 농어촌유학 / 말랑갯티학교란? 갯벌의 생명력을 담은…" |
| Claude Haiku 4.5 | 14,066ms | 357자 | 인식 | ✅ **정확** — 동일 내용 |
| **Nova Lite** | 9,105ms | **38자** | 빈 표 | 🔴 "2026년 초등학교 가정통신문" — **원문과 무관** |
| **Nova Pro** | 11,243ms | 465자 | 없음 | 🔴 "무료 한국어 학교 입학 안내문" — **완전 무관** |

같은 시험에서 확정된 사실 셋:

| 사실 | 내용 |
|---|---|
| **PDF 변환 불필요** | `Converse`가 PDF를 `document` 블록으로 **직접 받는다.** 1.2MB 통과 |
| 이미지 블록 | `image` 블록, `format: png\|jpeg` |
| 판독 지연 | Claude 계열 **14~16초** (PDF 1.2MB 기준) |

#### 5.9.3 왜 Nova를 뺐는가 — 이 제품에서 환각은 오류가 아니라 사고다

번역이 어색한 것과 **없는 내용을 지어내는 것**은 같은 종류의 실패가 아니다.
이 제품이 내보내는 글은 «학교가 학부모에게 보낸 가정통신문»으로 읽힌다. 화면에는
학교 이름과 공지 제목이 붙어 있고, 읽는 사람은 한국어를 편히 읽지 못해서 이 앱을 쓴다 —
**원문과 대조해 «이건 학교가 한 말이 아니다»라고 알아챌 수 있는 사람이 아니다.**
Nova Lite가 QR 코드 한 장을 받고 만들어낸 "교장 김 교장입니다"는 문장은 문법이 맞고
형식이 그럴듯해서 8축 평가에서 낮은 점수를 받지도 않는다. 그대로 나가면 제품이
**학교가 하지 않은 말을 학교 이름으로 전달**하게 된다. 준비물·날짜·비용을 지어내면
학부모가 실제로 잘못된 행동을 한다. 이건 품질 저하가 아니라 **north star(`README.md:7`)
정면 위반**이고, 되돌릴 방법이 없다 — 이미 읽힌 뒤다.

> **판정: Nova 계열은 사용자에게 보이는 한국어 텍스트를 생성하는 경로에서 제외한다.**
> 지연이 빠른 것(Nova Lite 341ms, 9.1초 PDF)은 사실이지만, **읽지 못했을 때 침묵하지
> 않는 모델**은 이 파이프라인의 어느 자리에도 그대로 놓을 수 없다. 완전 삭제가 아니라
> **용도 제한**이다 — §5.10.3.

### 5.10 모델 선택 정책

#### 5.10.1 Haiku 기본 → 필요할 때만 Sonnet 승급

| 순위 | 모델 | 언제 |
|---|---|---|
| **1 (기본)** | `global.anthropic.claude-haiku-4-5-20251001-v1:0` | **항상 여기서 시작한다.** PDF·이미지 판독 정확도가 Sonnet과 동급이고(§5.9.2) 텍스트 지연은 920ms vs 1,303ms로 더 짧다 |
| **2 (승급)** | `global.anthropic.claude-sonnet-4-6` | Haiku가 §4.6 게이트(G1~G6)를 **못 넘을 때만.** 승급은 «정말 필요할 때만» 한다 |
| 3 (혼합) | 기계 단계 Haiku + 번역 단계 Sonnet | 승급이 특정 단계에서만 필요할 때. `source_hard_fact_model` 분리로 가능 |

승급 판단은 **인상이 아니라 게이트 산출물로 한다.** Haiku arm이 G1~G6을 통과하면
Sonnet은 채택하지 않는다 — 더 비싸고 더 느린데 게이트가 구분하지 못하는 차이는
채택 근거가 되지 못한다. 승급도 되돌리기도 `BEDROCK_TRANSLATION_MODEL` **env 한 줄**이다.

#### 5.10.2 Opus 금지

**Claude Opus 계열은 이 사업의 어느 단계에서도 쓰지 않는다.** 사용자 지시이고
근거는 비용이다. 후보 표·arm 구성·설정 기본값·예시 코드 어디에도 넣지 않는다.
Haiku가 부족하면 **Sonnet까지가 상한**이고, Sonnet으로도 게이트를 못 넘으면
그때는 모델을 더 올리는 게 아니라 **프롬프트·단계 설계를 다시 본다**(별건).

#### 5.10.3 Nova — 삭제가 아니라 용도 제한

Nova는 싸고 빠르다. 버릴 이유는 없고, **놓을 자리가 제한될 뿐이다.**

| 쓸 수 있다 | 쓰지 않는다 |
|---|---|
| 출력이 **닫힌 집합**인 판정 — 분류·라우팅·플래그(«이 첨부는 식단표인가», «이 공지는 회신이 필요한가») | 사용자에게 보이는 **한국어 본문 생성** — 번역·요약·카드 문구 |
| 환각을 **기계적으로 검출할 수 있는** 자리 — 답이 열거형이라 집합 밖 값을 코드가 잡아낸다 | **문서 판독** — 못 읽으면 지어낸다는 것이 §5.9.2에서 확인됐다 |
| 결과가 틀려도 **다음 단계가 교정**하는 자리 | 검증·판정 단계 — 지어낸 근거로 통과시키면 G1이 못 잡는다 |

> 이 표의 기준은 «싸다/빠르다»가 아니라 **«이 자리에서 지어내면 코드가 알아챌 수 있는가»** 다.
> 알아챌 수 있으면 후보, 알아챌 수 없으면 제외.
> 이 사업(B = 번역)의 단계는 전부 후자다. → **B에서 Nova를 쓰는 자리는 없다.**
> 앞으로 분류·라우팅 단계가 생기면 그때 이 표를 근거로 다시 꺼낸다.

### 5.11 아직 확인되지 않은 제약 3건

| # | 제약 | 지금 아는 것 | 구현 시 해야 할 것 |
|---|---|---|---|
| **C1** | **요청 크기 상한** | Bedrock 요청에는 상한이 있다. 실측으로 **1.2MB PDF는 통과**했다. 현 파이프라인은 `MAX_FILE_SIZE_MB=50`까지 받는다 — 그 사이 어디에 벽이 있는지 **미확인** | **확인 Step을 반드시 앞에 둔다.** 상한을 실측한 뒤, 넘는 파일은 **쪼개거나 페이지를 고른다.** 추측으로 값을 박지 않는다 |
| **C2** | **판독 지연 14~16초** | PDF 1.2MB 기준 Sonnet 15.9초 / Haiku 14.1초. **현행 Gemini OCR 지연은 이 스펙에서 재지 않았다 — 미확인** | 같은 파일로 **Gemini 판독 지연을 실측해 나란히 비교**한다. Bedrock이 더 느릴 수 있고, 그러면 판독 이전(사업 C)의 손익이 뒤집힌다 |
| **C3** | **`global.` 라우팅 = 데이터가 APAC 밖으로** | 사용자 허용됨(§15 결정 5). 기술적 제약이 아니라 **거버넌스 기록** | 배포 로그에 어떤 추론 프로필을 호출했는지 남긴다. 허용 범위가 바뀌면 `apac.` 계열로 되돌릴 수 있어야 한다 |

> C1·C2는 **이 사업(B)의 번역 경로에는 당장 걸리지 않는다** — 번역 입력은 텍스트다.
> 걸리는 것은 판독(사업 C)이다. 여기서는 **접점만 기록**하고 넘긴다(§10.4).
...
### 10.4 문서판독도 Bedrock으로 옮길 수 있다 — 확인됨, 그러나 여기서 하지 않는다

**이 스펙의 전제 하나가 2026-08-27 실측으로 바뀌었다.**
원래 「번역만 Bedrock, 판독은 Gemini 유지」는 «판독은 Gemini에 남을 수밖에 없다»는
예비안이었다. 그 전제는 더 이상 유효하지 않다.

| 확인된 사실 | 근거 |
|---|---|
| Bedrock `Converse`가 **PDF를 `document` 블록으로 직접 받는다** (변환 불필요, 1.2MB 통과) | §5.9.2 |
| 이미지는 `image` 블록(`format: png\|jpeg`)으로 받는다 | §5.9.2 |
| Claude Haiku 4.5 / Sonnet 4.6이 **실제 학교 첨부의 한글·표를 정확히 읽는다** | §5.9.2 시험 2 |
| 못 읽는 입력(QR 코드)에는 **정직하게 못 읽겠다고 답한다** | §5.9.2 시험 1 |

즉 「번역만 이전」은 **기술적 한계가 아니라 사업 경계**다.

> **그래도 이 사업에서는 옮기지 않는다.** 판독 이전은 **사업 C 영역**이다.
> 여기서는 «가능함이 확인됐다»는 사실과 접점 셋만 기록하고 넘긴다:
>
> 1. **요청 크기 상한(§5.11 C1)** — `MAX_FILE_SIZE_MB=50`과의 간격을 C가 실측해야 한다
> 2. **판독 지연 비교(§5.11 C2)** — Bedrock 14~16초 vs 현행 Gemini OCR **미실측**
> 3. **판독에도 §5.10 정책이 그대로 적용된다** — Haiku 기본, Sonnet 승급, Opus 금지,
>    **Nova는 판독에 절대 넣지 않는다**(§5.9.2가 정확히 판독에서 지어냈다)
>
> 판독이 C로 옮겨가면 §9.2의 SDK 결함 셋과 열린 질문 Q3도 함께 닫힌다.
...
| **4** | **B4** thinking 축소 (Gemini) | **§4.6 G1~G6** | 단건 지연 134.7 → 33.5s 근처 |
| **5** | **B5** Bedrock 클라이언트 (다크) | — | `TRANSLATION_BACKEND=gemini`로 배포 → 런타임 동작 무변화 |
| **6** | **B6** 워커만 Bedrock | **§4.6 G1~G6** (Bedrock arm) | 워커 로그의 `label=translation:…claude-haiku-4-5…`, 크레딧 차감 확인 |
| **7** | **B7** 전면 Bedrock | 6번 무사고 7일 | Gemini 호출이 문서판독·크롤러에만 남음 |
| **8** | **B9** 155건 백필 | — | 재생성 후 `validation.hard_fact` 통과율 비교 |
| **9** | **B8** 단계 병합 M1+M2 | **§4.6 G1~G6** | 라운드 7 → 5, 지연 재측정 |
...
| **환각 G6** | negative control 입력에 대한 응답 (§4.7) | **지어낸 본문 0건.** 전부 «못 읽겠다» 계열 응답 |
| B6 결제 | Bedrock 콘솔 사용량 | 크레딧 차감, 현금 $0 |
| B6 모델 | 호출한 모델 문자열 | **Haiku 기본**(§5.10.1). Sonnet이 찍히면 승급 근거가 게이트 산출물에 있어야 한다. **Opus·Nova는 0건** |
| B6 라우팅 | 호출한 추론 프로필 문자열 | `global.`·`apac.` 모두 허용(§16 해결됨 Q1). 어느 쪽을 썼는지 **로그로 남는다** |
| B7 잔여 | Gemini 호출 로그 | 문서판독·크롤러만 남음 |
| B8 | 라운드 수 + §4.6 | 7 → 5, G1~G6 통과 |
...
| **모델이 읽지 못한 문서의 본문을 지어낸다** | **학교가 하지 않은 말이 학교 이름으로 학부모에게 간다.** 원문 대조가 어려운 사용자라 검출되지 않는다 | §5.9.2에서 Nova Lite·Nova Pro가 실제로 그랬다 → 한국어 생성 경로에서 제외(§5.10.3). Claude 계열은 정직하게 거부. **G6 negative control**로 arm마다 재검(§4.7) |
| **`global.` 라우팅으로 학생 정보가 APAC 밖으로** | 개인정보 이슈 | **사용자 허용됨**(§15 결정 5). 완화가 아니라 **기록**이다 — 호출한 추론 프로필을 로그에 남기고, 허용 범위가 바뀌면 `apac.` 계열로 되돌린다(§5.11 C3) |
| **첨부가 Bedrock 요청 상한을 넘는다** | 판독 요청이 통째로 실패 | 번역 경로(이 사업)는 텍스트라 걸리지 않는다. 판독(사업 C)에서 **상한 실측 Step 선행**(§5.11 C1) |
...
| **JSON 프리필이 모델에서 안 먹는다** | 파싱 실패 → 잡 실패 | B1 평가 단계에서 arm별 파싱 실패 건수로 사전 발견. 프리필을 무시하고 완전한 JSON을 주는 모델을 위해 이미 `{`로 열려 있으면 재부착하지 않는다 |
...
| 3 | Bedrock에서 Claude를 써도 되는가 | **써도 된다 — 사용자 명시 승인("Bedrock은 AWS니까").** | §4.3 arm 구성, §5.9 후보표 |
| 4 | 서버도 AWS로 옮기는가 | **옮기지 않는다.** GCP Cloud Run에 그대로 둔다. **인프라 이전이 아니라 모델 교체다** | §2 비목표, §5.7(GCP에서 AWS 자격증명을 얻는 문제가 여기서 생긴다) |
| 5 | `global.` 라우팅을 허용하는가 | **허용한다 — 사용자 승인.** 그래서 Claude 4.x 계열이 후보가 되고 기본값이 Haiku 4.5로 잡힌다 | §5.9.1, §5.10.1, §16 해결됨 |
| 6 | 어느 Claude를 기본으로 두는가 | **Haiku 기본, Sonnet은 «정말 필요할 때만» 승급. Opus는 금지(비용) — 사용자 지시** | §5.10.1, §5.10.2, §4.3 arm 순서 |
| 7 | Nova를 쓰는가 | **한국어 본문 생성·판독에서 제외.** 실측에서 없는 본문을 지어냈다. 다만 삭제가 아니라 **용도 제한** — 출력이 닫혀 있고 환각을 코드가 검출할 수 있는 자리에서만 후보 | §5.9.2, §5.9.3, §5.10.3 |
...
| # | 질문 | 답 (2026-08-27) | 결과 |
|---|---|---|---|
| **Q1** | **`global.` 라우팅을 허용하는가?** 학교 공지에는 학생 이름·학년반·보호자 연락처가 섞인다. `global.` 접두는 요청이 APAC 밖으로 나갈 수 있다는 뜻이다 | **해결됨 — 허용.** 사용자 승인(§15 결정 5) | `apac.` 전용 제약이 풀린다. Claude 4.x 계열이 후보가 되고 기본값이 **Haiku 4.5**(§5.10.1)로 잡힌다. 남는 것은 «어느 프로필을 호출했는지 로그로 남긴다»는 기록 의무뿐(§5.11 C3) |
...
| **Q3** | **문서판독(멀티모달)은 언제 옮기는가?** «옮길 수 있는가»는 더 이상 질문이 아니다 — 2026-08-27 실측으로 **가능함이 확인됐다**(§10.4). 남은 것은 **시점과 주체**다 | 옮기지 않으면 SDK 결함 #2705·#2869가 영구히 남고(§9.2), Gemini 현금 지출도 완전히는 사라지지 않는다 | **사업 C.** 착수 시 §5.11의 C1(요청 크기 상한)·C2(판독 지연 비교)를 실측 Step으로 앞에 둔다 |
