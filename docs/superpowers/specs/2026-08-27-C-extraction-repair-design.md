...
|---|---|---|---|
| 1 | 본문 사진 0건의 진짜 원인은? | ~~H0~H5 중 어느 것도 코드만으로는 판정 불가~~ **해소(2026-08-27, Task 9)**: 판정 D2. 운영 35건 전부 기능 도입 창(2026-06-04) 이후 추출, 전부 `status='done'`. 미보유 24건 전부 `sources[]`에 `inline_image` 타입 소스가 0개 — 즉 본문에 사진이 애초에 없는 정상 공지다(손실 후보 0건). 사진이 있는 11건은 전부 `body_images_combined` 보유(11/11 = 100%). 회수율 31%는 고장이 아니라 분모(전체 공지)를 잘못 잡은 착시였다. Task 6(성공판정 게이트 완화)은 불필요 — 건너뜀 | 완료: `scripts/diagnose_body_images.py` |
