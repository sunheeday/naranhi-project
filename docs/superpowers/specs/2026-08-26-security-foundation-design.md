...
> ⚠️ **이 라우트가 프로덕션에서 열려 있으면 A2 전체가 무의미해진다.**
> `TEST_ENTRY_BYPASS`를 끄고 `/home`을 열어두는 것은 같은 구멍을 이름만 바꾼 것이다.
> 기본값은 **꺼짐**으로 하고, 프로덕션에서 켤지는 §8 열린 질문 1번.
...
**(e) 데모 학교 예외.**
`lib/demo-school.ts:966`이 읽는 데모 첨부는 학부모 스코프 검사를 통과할 수 없다
(데모 학교에 실제 자녀가 없으므로). 데모 경로는 **별도 분기**로 두거나,
데모 학교를 스코프 검사의 예외로 명시한다. → §8 열린 질문 3번.
...
1. **`/home` 개발 진입로를 프로덕션에서도 켤 것인가?**
   켜면 인증 복구의 실효가 사라진다. 끄면 시연 때마다 실제 구글 계정이 필요하다.
   대안: 프로덕션에서는 끄고, 시연용 구글 계정을 미리 만들어 온보딩까지 끝내둔다.

2. **Supabase refresh token 만료가 현재 며칠인가?**
   대시보드 확인이 필요하다. 14일보다 짧으면 사용자가 조정해야 한다.

3. **데모 학교 첨부를 학부모 스코프의 예외로 둘 것인가?**
   데모 학교(`Naranhi School`)에는 실제 자녀가 없어 스코프 검사를 통과할 수 없다.
   예외로 두면 검사에 구멍이 생기고, 안 두면 데모에서 첨부가 안 보인다.
   사업 D의 데모 데이터 분리와 함께 결정하는 편이 나을 수 있다.

4. **기존 첨부 중 `storage_path` 역산 실패분을 어떻게 할 것인가?**
   재추출(비용·시간 발생) vs 해당 첨부 포기. 실측상 최대 11건 규모.
