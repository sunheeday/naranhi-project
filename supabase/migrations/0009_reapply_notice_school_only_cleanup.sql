...
-- Re-apply the intended 0006 cleanup in case the remote history row stored
-- the wrong migration body. All statements below are idempotent.
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      ...
      and policyname = 'notice cards select own school'
  ) then
...
do $migration_history_fix$
declare
  migration_sql text := $expected_0006$
begin;
...
begin
  if exists (
    ...
    set
      name = 'notices_school_only_cleanup',
      statements = array[migration_sql]
    where version = '0006';
  end if;
end;
$migration_history_fix$;
