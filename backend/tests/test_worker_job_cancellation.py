...
class WorkerCancellationTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_translation_worker_processes_claimed_jobs_concurrently(self) -> None:
        class _ConcurrentQueue:
            def __init__(self) -> None:
                self._first = True
                self.completed: list[str] = []
            ...
            def claim(self, *, job_types, limit):  # noqa: ANN001, ARG002
                ...
                self._first = False
                return [
                    {"id": "job-1", "payload": {"notice_id": "n1", "target_language": "ru"}},
                    {"id": "job-2", "payload": {"notice_id": "n2", "target_language": "ru"}},
                ][:limit]
        ...
        with (
            ...
            task = asyncio.create_task(
                process_translation_jobs(max_jobs=2, batch_size=2, retry_delay_seconds=120, stale_seconds=3600)
            )
            await asyncio.wait_for(both_started.wait(), timeout=1.0)
            release.set()
            processed = await asyncio.wait_for(task, timeout=1.0)
